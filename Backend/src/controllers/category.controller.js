...
const CategoryController = {
    ...
    update3D: asyncHandler(async (req, res) => {
        ...
        const updateFields = {};
        if (rawdata.scale !== undefined) {
            updateFields['three_d_config.scale'] = parseFloat(rawdata.scale);
        }
        if (rawdata.position) {
            if (rawdata.position.x !== undefined) updateFields['three_d_config.position.x'] = parseFloat(rawdata.position.x);
            if (rawdata.position.y !== undefined) updateFields['three_d_config.position.y'] = parseFloat(rawdata.position.y);
            if (rawdata.position.z !== undefined) updateFields['three_d_config.position.z'] = parseFloat(rawdata.position.z);
        }
        if (rawdata.rotation) {
            if (rawdata.rotation.x !== undefined) updateFields['three_d_config.rotation.x'] = parseFloat(rawdata.rotation.x);
            if (rawdata.rotation.y !== undefined) updateFields['three_d_config.rotation.y'] = parseFloat(rawdata.rotation.y);
            if (rawdata.rotation.z !== undefined) updateFields['three_d_config.rotation.z'] = parseFloat(rawdata.rotation.z);
        }

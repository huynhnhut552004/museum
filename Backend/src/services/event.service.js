...
const EventService = {
  getEvents: async ({ type = 'all', page = 1, limit = 10 }) => {
    ...
    const query = `
      SELECT id, title, slug, banner_url, description, start_time, end_time, total_views
      FROM events
      ${whereClause}
  ...
  getEventDetail: async (slug) => {
    ...
    if (res.rows.length === 0) {
        const check = await pool.query('SELECT * FROM events WHERE slug = $1', [slug]);
        return check.rows[0];
    }
    return res.rows[0];
  },

...
export default function ZoomableImage({img, title, desc, hotspots}) {
  ...
  return (
    <AnimatedSection>
      <div className='lg:hidden pb-2'>
        ...
        <Link to="#">
          <div className='Style-Text1 border-black underline text-center rounded-lg lg:hidden '>Khám phá thêm</div>
        </Link>
      </div>
    <section className="relative pb-10 w-full lg:h-[90vh] h-[30vh] overflow-hidden group rounded-xl">
      ...
      <div className={`absolute bottom-6 lg:block hidden left-0 p-8 text-white transition-opacity duration-500 ${isZoomed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        ...
        <Link to="#">
          <div className='Style-Text1 text-black hidden bg-white p-1 cursor-pointer rounded-lg transition-all duration-300 ease-out lg:inline-block hover:bg-gray-300 '>Khám phá thêm</div>
        </Link>
      </div>
